...
namespace our {
    ...
    public:
        ...
        Texture2D() {
            ...
            glGenTextures(1, &name);   // glGenTextures(#textures, &textureName)
        };

        // This deconstructor deletes the underlying OpenGL texture
        ~Texture2D() { 
            //TODO: (Req 5) Complete this function
            glDeleteTextures(1, &name);  // glDeleteTextures(#textures, &textureName)
        }
        ...
        void bind() const {
            //TODO: (Req 5) Complete this function
            glBindTexture(GL_TEXTURE_2D, name); // glBindTexture(GL_TEXTURE_2D, textureName)
        }

        // This static method ensures that no texture is bound to GL_TEXTURE_2D
        static void unbind(){
            //TODO: (Req 5) Complete this function
            glBindTexture(GL_TEXTURE_2D, 0); //glBindTexture(GL_TEXTURE_2D, 0) , 0 means unbind
        }
